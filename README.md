...
- **图文动态图片** - 提取图文动态/文章中的图片 URL，注入图片数量到 prompt（**图片 URL 尚未传给 LLM**，等 DeepSeek 支持视觉后对接）
...
GenerateStage   → PydanticAI Agent 会话级调用（Agent 内部维护历史，session_key 关联）→ 失败降级 Provider.generate()
...
用户评论 "总结一下这个视频"
    ↓
GenerateStage → ProviderManager.chat(session_key, user_message)
    ↓
Agent（按会话缓存）
